...
export class TranscriptService {
  async fetchTranscriptData(youtubeId: string): Promise<TranscriptData> {
    try {
      ...
      if (Array.isArray(response)) {
        ...
        segments = response;
      } else if (response && (response as any).segments && Array.isArray((response as any).segments)) {
  ...
  chunkTranscript(transcriptData: TranscriptData, chunkSize: number = 500, overlap: number = 50): Array<{
    content: string;
    startSec?: number;
    endSec?: number;
  }> {
